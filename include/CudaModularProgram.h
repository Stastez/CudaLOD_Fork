	...
	// CudaModularProgram(vector<string> modulePaths, vector<string> kernelNames = {}){
		...
		for(auto modulePath : modulePaths){
			...
			monitorFile(modulePath, [&, module]() {
				...
				string newSource = readFile(module->path);

				// only compile&link when main kernel file was actually modified
				// note that this misses changes in included files
				// if(newSource != module->source){
				// 	// recompile
				// 	module->compile();
				// 	link();
				// }else{
				// 	// no changes
				// 	cout << "source files did not change" << endl;
				// }

				// allways compile&link on ctrl+s, even if there are no modifications
				module->compile();
				link();
